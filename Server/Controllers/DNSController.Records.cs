...
public partial class DNSController
{
    ...
    public async Task<ActionResult<IResponse>> GetRecord(string qName, string qType)
    {
        List<Record>? foundRecords;
        if (qType.Equals("ANY", StringComparison.OrdinalIgnoreCase))
            foundRecords = await _recordInfoService.ListRecordAsync(qName);
        else
            foundRecords = await _recordInfoService.GetRecordAsync(qName, qType);
    ...
    public async Task<ActionResult<IResponse>> GetRecordById(string recordId)
    {
        var foundRecords = await _recordInfoService.GetRecordByIdAsync(recordId);
    ...
    public async Task<ActionResult<IResponse>> ListRecords(uint zoneId)
    {
        var foundRecords = await _recordInfoService.ListRecordByZoneIdAsync(zoneId);
        if (foundRecords != null) return Ok(new DataResponse<List<Record>>(foundRecords));
        return Ok(new DataResponse<List<Record>>(new List<Record>()));
    }
    ...
    [HttpPost("Record")]
    public async Task<ActionResult<IResponse>> NewRecord([FromBody] Record record)
    {
        var tryCreate = await _recordInfoService.SetRecordAsync(record);
    ...
    [HttpDelete("Record")]
    public async Task<ActionResult<IResponse>> DeleteRecord(Record record)
    {
        var tryDelete = await _recordInfoService.DeleteRecordAsync(record);
